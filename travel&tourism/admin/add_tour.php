...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["featured_image"]) && $_FILES["featured_image"]["error"] == 0) {
        $upload_dir_relative = 'assets/images/tours/';
        ...
        $image_file_type = strtolower(pathinfo($_FILES["featured_image"]["name"], PATHINFO_EXTENSION));
        $new_file_name = uniqid('tour_') . '.' . $image_file_type;
        $target_file_absolute = $upload_dir_absolute . $new_file_name;
        $target_file_relative = $upload_dir_relative . $new_file_name;
        ...
        if (empty($errors)) {
            if (move_uploaded_file($_FILES["featured_image"]["tmp_name"], $target_file_absolute)) {
                $featured_image = $target_file_relative;
            } else {
...
<head>
    ...
    <title>Add New Tour - Travel Management System</title>
